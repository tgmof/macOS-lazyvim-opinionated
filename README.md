...
It was successfully tested in MacOS Sequoia (15).
For the Windows-WSL setup, see <https://github.com/tgmof/wsl-lazyvim-opinionated>

The installation bash file installs Alacritty in MacOS. This is less fancy than (for instance) WezTerm/Ghostty but this is on purpose: They would add yet another layer of complexity for neovim/lazyvim beginners and yet another set of keyboard shortcut to understand / learn, and Alacritty has the additional advantage of using GPU acceleration a lot and having a very low RAM footprint (order of 30MB against 300MB for WezTerm for instance).

The color mode (dark/light) follows Windows color mode (System Settings > Appearance > dark or light).
As of time of writing, the dark-notify neovim plugin seem to have a small bug because the buffer row doesn't update the theme color at the top of the screen.
...
```bash
cd
git clone https://github.com/tgmof/macOS-Lazyvim-opinionated.git
cd macOS-lazyvim-opinionated && ./neovim_for_macOS.sh # sh script is idempotent, so you can run it twice if needed
cd
# rm -rf macOS-lazyvim-opinionated # this also deletes your neovim config backup...
nvim
```

Learn the following keybindings by heart for quick start: (note that "+" means that you end up with several keys pressed AT THE SAME TIME whereas the absence of "+" means "press, release and then continue with the next key". Ex: `<Ctrl>+c` (concurrent) is different from `<Ctrl>c` (sequential))
